...
<body>
  ...
  <?php
    ...
    $sql = $con->prepare("SELECT * FROM tools WHERE id = :tool_id");
    $sql->execute([':tool_id' => intval($tool)]);
            ...
            echo ", $category2";
                ...
                <div class="collapse options" id="options">
                  ...
                  <div class="col-xl-12">
                    <div class="row">
                        <?php
                          $sql2 = $con->prepare("SELECT name, example, command FROM commands WHERE tool = :tool_id AND type='input' ORDER BY name");
                          $sql2->execute([':tool_id' => intval($tool)]);
                        ...
                        <?php
                          $sql2 = $con->prepare("SELECT id, name, command FROM commands WHERE tool = :tool_id AND type='checkbox' ORDER BY name");
                          $sql2->execute([':tool_id' => intval($tool)]);
                          $resultados2 = $sql2->fetchAll(PDO::FETCH_ASSOC);
                          // FOREACH BEGINS
                          foreach ($resultados2 as $resultado2) {
